...
class empleadoDAO(DBConfig):

    def __init__(self):
        super().__init__()

        # Clave para encriptar/desencriptar
        self.clave_fernet = b"5Jp-fpyG6Cx8bgaPeBMWQtCng_Gjr6eUmaTvu3R5IUs="
                ...
                if rs is None:
                    ...
                    if not contrasena_bd_encriptada: # Si no hay contraseña en la BD
                        ...
                        try:
                            ...
                            print(f"Error al desencriptar la contraseña (DAO): {e}")
                            return None # Si no se puede desencriptar, el login falla y retorna None 

                        # 4. Comparar la contraseña desencriptada con la que ingresó el usuario
                        if contrasena_bd_desencriptada == contrasena:
                            # Creamos y devolvemos el objeto empleado con sus datos más el tipo de acceso para darle 
                            # Los permisos correspondientes según su rol
                            emp = empleado()
                            emp.setRut(rs[1])
                            emp.setNombres(rs[2])
...
#----------------------------------------------------------------------------------------------
# BUSCAR EMPLEADOS EXISTENTES (HABILITADOS)
